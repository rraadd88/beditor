...
# from beditor.lib.utils import rescale
...
def get_ppamdist(
    ...
    :param guide_length: length of guide sequence
    :param pam_len: length of PAM sequence
    :param pam_pos: PAM location 3' or 5'
    :param ppamdist_min: minimum penalty
    :param pmutatpam: penalty for mismatch at PAM

    TODOs:
        Use different scoring function for different methods.
    """
    from roux.stat.transform import rescale

    x = np.arange(pam_len)

    guide_positions = np.arange(guide_length)
    guide_positions = rescale(guide_positions, range2=[0, 1])
    coeffs = np.array([0.86947513, -1.15561009, -0.0826878, 0.77644198])  ## optimized
    ppamdist = np.polyval(coeffs, guide_positions)
    # ppamdist=rescale(ppamdist,mn=ppamdist_min)
    # ppamdist=rescale(ppamdist,range2=[1,ppamdist_min])
    ppamdist = rescale(ppamdist, range2=[ppamdist_min, 1])
    # ppamdist=np.concatenate((ppamdist,ppamdist_pam))
    #     ppamdist=1-ppamdist
    if pam_pos == "up":
        ppamdist = ppamdist[::-1]
    elif pam_pos == "down":  # NGG
        ppamdist = ppamdist
    else:
...
def get_beditorscore_per_alignment(
    ...
    """
    ## constant
    if pd.isnull(alignment):
        raise ValueError("alignment is None")
    if NM == 0:
        return 1

    ## penalty for not perfect alignment
    ### weights
    dist_from_pam_penalties = get_ppamdist(
        guide_length=len(alignment),
        pam_len=pam_len,
        pam_pos=pam_pos,
    ...
    mutations_penalties_multi = mutations_penalties_multi[
        mutations_penalties_multi != 0
    ]

    assert len(mutations_penalties_multi) != 0, len(mutations_penalties_multi)

    penality_cum_dist_from_pam = np.prod(mutations_penalties_multi)

    assert penality_cum_dist_from_pam != 0, penality_cum_dist_from_pam

    if verbose:
        logging.info(
            (
                f"dist_from_pam_penalties={dist_from_pam_penalties}\n"
                f"mutations_penalties={mutations_penalties}\n"
                f"mutations_penalties_multi={mutations_penalties_multi}\n"
                f"penality_cum_dist_from_pam={penality_cum_dist_from_pam}\n"
            )
        )

    return 1 - penality_cum_dist_from_pam


def get_beditorscore_per_guide(
    guide_seq: str,
    strategy: str,
    align_seqs_scores: pd.DataFrame,
    dBEs: pd.DataFrame,
    ...
    pos_muts = (
        dBEs.loc[
            ...
            [
                ...
                "distance of codon start from PAM: maximum",
            ],
        ]
        .drop_duplicates()
        .set_index("method")
    ...
    penalty_activity_window = (
        1
        if (
            pos_muts.loc[method, "distance of mutation from PAM: minimum"]
            <= pos_mut
            <= pos_muts.loc[method, "distance of mutation from PAM: maximum"]
        )
        else penalty_activity_window
    )
