...
def din2dseq(cfg):
    ...
    if not exists(dseqp) or cfg['force']:
        ...
        cols_dseq=['aminoacid: position', 'gene: id', 'gene: name', 'protein: id', 'transcript: id', 'transcript: sequence', 'aminoacid: wild-type', 'codon: wild-type', 'contig', 'strand', 'start', 'end', 'codon start', 'codon end']
        if not all([True if c in din else False for c in cols_dseq]):
